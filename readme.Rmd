---
title: "Replication materials for ``Winners, Losers, and Referendums: The Impact of Brexit on Political Attitudes''"
...
2) `data_tidying_panel.R` converts the data into from wide into long format, which is required for the analysis. I also scale the most variables to range from 0-1 for ease of interpretation. 
...
3) `models.R` runs all of the models included in the paper. This file also produces (1) the LaTeX regression output used in the paper (using `texreg`) and (2) marginal effects plots.